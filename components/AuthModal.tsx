...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose }) => {
  ...
  const checkEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If we can't check (e.g. permissions), default to login
      // If the user doesn't exist, signInWithEmailAndPassword will fail with auth/user-not-found
      // and we can handle it there.
      setIsLogin(true);
      setStep('auth');
    } finally {
      setLoading(false);
    ...
    try {
      ...
      if (isLogin) {
        try {
          ...
          // If user not found in Auth but we thought it was a login
  ...
  // Prefill email / open directly to auth step when requested via localStorage
  React.useEffect(() => {
    if (isOpen) {
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[999] grid place-items-center p-4 overflow-y-auto">
          ...
          <motion.div 
            ...
            className="relative w-full max-w-md bg-white border border-slate-100 rounded-[2rem] overflow-hidden shadow-[2xl] my-auto flex flex-col"
          >
            {/* Green Gradient Header */}
            <div className="relative w-full bg-gradient-to-br from-slate-900 via-emerald-900 to-teal-900 pt-10 pb-8 px-8 sm:px-10 text-center overflow-hidden">
               <div className="absolute top-0 right-0 w-48 h-48 bg-emerald-500 rounded-full blur-[50px] opacity-20 -translate-y-1/2 translate-x-1/3 pointer-events-none" />
               <div className="absolute bottom-0 left-0 w-48 h-48 bg-teal-500 rounded-full blur-[50px] opacity-20 translate-y-1/3 -translate-x-1/4 pointer-events-none" />
               
               <button 
                 aria-label="Fechar autenticação"
                 onClick={onClose}
                 className="absolute top-4 right-4 p-2 text-white/50 hover:text-white transition-colors z-10 bg-white/5 rounded-full backdrop-blur-md"
               >
                 <X size={18} />
               </button>

               <div className="relative z-10 space-y-2">
                 <h2 className="text-3xl font-black text-white tracking-tighter uppercase italic drop-shadow-md">
                   {step === 'email' ? 'Portal do Cliente' : (isLogin ? 'Bem-vindo' : 'Crie sua conta')}
                 </h2>
                    ...
                    <form onSubmit={checkEmail} className="space-y-4">
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full bg-slate-50 border border-slate-200 rounded-2xl py-5 pl-14 pr-4 text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all font-medium text-sm"
                        />
                      </div>
                      ...
                      <button
                        type="submit"
                        disabled={loading}
                        className="w-full bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white font-bold py-5 rounded-2xl shadow-xl shadow-emerald-600/20 flex items-center justify-center space-x-3 transition-all active:scale-95 mt-6"
                      >
                        {loading ? (
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    {isLogin && (
                      <button 
                        ...
                        className="w-full text-[9px] font-bold text-slate-400 hover:text-emerald-600 uppercase tracking-widest transition-colors pt-4 pb-2"
                      >
                        Esqueceu sua senha?
                      </button>
                    )}
                    ...
                    <button 
                      ...
                      className="w-full text-[9px] font-bold text-slate-400 hover:text-slate-900 uppercase tracking-widest transition-colors pt-2"
                    >
