...
const ClientPortal: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Two targeted queries compatible with strict Firebase security rules
        // 1) simulations where userId already linked
        const q1 = query(collection(db, 'simulations'), where('userId', '==', user.uid));
        // 2) simulations where email matches (orphans not yet linked)
        const q2 = query(collection(db, 'simulations'), where('userEmail', '==', (user.email || '').toLowerCase().trim()));
        ...
        [...snap1.docs, ...snap2.docs].forEach(d => {
          if (!seen.has(d.id)) {
            seen.add(d.id);
            simData.push({ id: d.id, ...d.data() } as Simulation);
          }
        });
        ...
        const letterSnapshot = await getDocs(
              ...
              {letters.map((letter) => (
                <div key={letter.id} className="bg-white border border-slate-100 p-3 rounded-xl flex items-center justify-between text-sm shadow-sm hover:border-emerald-200 transition-colors cursor-pointer" onClick={() => window.location.href='/cartas'}>
                  <div>
                    ...
                    <div className="text-xs text-emerald-600 font-black mt-0.5">{formatCurrency(letter.credit)}</div>
                  </div>
                  <div className="text-right flex items-center gap-1 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                    Ver <ChevronRight size={14} className="text-emerald-500" />
                  </div>
                </div>
              ))}
            </div>
