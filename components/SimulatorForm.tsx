...
import { Home, DollarSign, Leaf, Car, ArrowRight, CheckCircle2, Building2, Palmtree, Hammer, Key, Map as MapIcon, X, Truck } from 'lucide-react';
...
const SimulatorForm: React.FC<SimulatorFormProps> = ({ onSuccess }) => {
  const [type, setType] = useState<ConsortiumType>('Imóvel');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Save email for quick registration on Thank You page
  ...
  const consortiumTypes: { id: ConsortiumType; icon: any; label: string }[] = [
    { id: 'Imóvel', icon: Building2, label: 'Imóvel' }
  ];
      ...
      >
        ...
        >
          ...
          <div className="space-y-6">
            {/** Step 1: Slider + Type Selection (shown when showFullForm === false) */}
            {!showFullForm ? (
              <>
                {/* Credit Value Slider */}
                ...
                {/* Consortium Type Selection Omitted as it is now only Imovel */}
              </>
            ) : (
              /** Step 2: Contact Info replaces step 1 */
              <div className="space-y-4">
